...
def collect_files(directory: str, name_filter: str = "", recursive: bool = True) -> list[str]:
    files: list[str] = []
    if recursive:
...
def read_files_parallel(file_paths: list[str], max_workers: int | None = None) -> list[FileContent]:
...
def collect_and_read_files(
    ...
    return read_files_parallel(collect_files(directory, name_filter, recursive), max_workers)
